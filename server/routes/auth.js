...
router.get('/me', (req, res) => {
  ...
  try {
    ...
    db.get(`SELECT id, name, email, rollNo, sapId, university, department, course, shift, section, semester, image, reason, qualities, isAdmin, approvalStatus, rejectionReason FROM users WHERE id = ?`, [decoded.userId], (err, user) => {
      if (err) return res.status(500).json({ error: err.message });
